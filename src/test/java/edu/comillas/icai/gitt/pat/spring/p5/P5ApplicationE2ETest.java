...
class P5ApplicationE2ETest {
    ...
    @Test public void loginOkTest() {
        ...
        // When ...
        ResponseEntity<String> responseLogin = client.exchange(
                "http://localhost:8080/api/users/me/session",
                HttpMethod.POST, new HttpEntity<>(login, headers), String.class);

        // Then ...
        List<String> header = responseLogin.getHeaders().get("set-cookie");

        Assertions.assertEquals(HttpStatus.CREATED, response.getStatusCode());
        Assertions.assertNotNull(header);
        Assertions.assertEquals(1, header.size());
        Assertions.assertTrue(header.get(0).startsWith("session="));
